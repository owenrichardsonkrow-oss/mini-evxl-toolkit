...
const MiniEvxlEngine = (function(){
  ...
  //
  // AGGRESSIVE, and what that costs when replayed over his 1,238 real bouts that had a PB to
  // beat (median history 3 runs). THE SHIPPED RULE IS 0.685 WITH THE TWO-RUN FLOOR (step 36):
  // 194 items close on stagnation against 573 on a PB, at a median of TWO runs before a stop,
  // and 4.2% of all items (52) close before a PB his actual play went on to get. The floor
  // recovered exactly the 48 run-2 PBs the threshold move had given up -- rule-reached arrival
  // went 468/51/6 back to 468/99/6 -- so the shipped operating point costs LESS than the old
  // 0.6 did without it (244 / 550 / 6.1% (75)); pre-floor 0.685 read 321 / 525 / 8.1% (100).
  //
  ...
  // It counts every PB EVENT in the record (625), not the 573 the rule actually reaches. The
  // difference is exactly the 52 early-closes, and by construction those are all LATE PBs --
  ...
  //                          run 2         99       (15.8%)          +4.17%
  //                          run 3+        58        (9.3%)          +2.36%
  //     of which the RULE reaches:        468 / 99 / 6 (573), gains +6.23% / +4.17% / +2.15%
  ...
  // AN ITEM GETS AT LEAST TWO RUNS THIS VISIT before exhaustion may close it (step 36, D37).
  // The owner's first real feel report (2026-08-26): one non-PB run on a one-prior-run scenario
  // moved on immediately -- n/(n+k) = 1/2 <= 0.685 fires at k = 1 for every n <= 2, and it fired
  // at the old 0.6 for n = 1 too, so this is not the threshold move's fault. Measured on the
  // committed replay before deciding (dev/hazard-lambda.json /minKFloor): the floor recovers 48
  // PBs (525 -> 573) and HALVES the early-close rate (8.1% -> 4.2%) at the cost of one extra run
  // on items that stop; k >= 3 recovers 76 but guts the rule (321 -> 114 stops), so 2 it is.
  // ANYTIME VALIDITY IS UNTOUCHED: the events {no PB by k} stay nested, so P(ever stopping) is
  // P(no PB by the first QUALIFYING k), which a floor only lowers. A POLICY bound, like
  // FEEL_ADJ_MAX -- the aggressive threshold stays; what changes is the minimum evidence per
  // visit before it may act. The form test and the 'too hard' exit are deliberately NOT floored:
  // form needs minHistory anyway, and a rating is the player speaking.
  const STAGNATE_MIN_K = 2;
  ...
  function stagnation(history, bout, pbAt, opts){
    const o = Object.assign({ stagnateAt: STAGNATE_AT, stagnateMinK: STAGNATE_MIN_K, formAlpha: FORM_ALPHA, minHistory: FORM_MIN_HISTORY, lambda: 1 }, opts||{});
    ...
    for(let i=0;i<runs.length;i++){
      ...
      if(k >= o.stagnateMinK && pEx <= o.stagnateAt) return { done: true, why: 'exhausted', k, runs: runs.length, n, pExhaust: pEx, martingale };
    }
  ...
  //   3. ADAPTIVE TEMPLATE by profile confidence (three bands; mid = 6/2/1/1 since D39).
  ...
  // D39 (2026-08-27): the QUICK-WIN purpose is retired from the coach. Tier-proximity
  // ("76% of the way to the next tier") is benchmark-ladder logic -- D35 says score is what a
  // player chasing a RANK cares about -- and the coach's objective is the FLOOR (D34). The two
  // aims conflict in the same slot, and the owner called it carryover from the ten-item format.
  // Its weight moves to WEAKEST, per D34. The Home "Quick wins" panel stays: that surface IS
  // the benchmark ladder. Historical ledger rows with why:'quickwin' still render (the UI keeps
  // the label); nothing serves the purpose again.
  const SESSION_TEMPLATE = { weakest: 6, route: 2, fillout: 1, revisit: 1 };
  ...
  const SESSION_TEMPLATES = {
    low:  { weakest: 5, route: 2, fillout: 2, revisit: 1 },
    mid:  SESSION_TEMPLATE,
    high: { weakest: 7, route: 1, fillout: 0, revisit: 2 }
  };
  ...
  const SESSION_TYPES = {
    floor:    { weakest: 7, route: 1, fillout: 0, revisit: 2 },
    transfer: { weakest: 4, route: 4, fillout: 0, revisit: 2 },
    collect:  { weakest: 4, route: 1, fillout: 0, revisit: 5 },
    breadth:  { weakest: 4, route: 1, fillout: 4, revisit: 1 }
  };
  ...
  const PURPOSES = ['weakest', 'route', 'fillout', 'revisit', 'coverage'];  // quickwin retired, D39
  ...
  function sessionTemplate(c, size){
    ...
    const keys = ['weakest', 'route', 'fillout', 'revisit'];
  ...
  function composeSession(scenarios, opts, playlistFill){
    ...
    // (D39: the QUICK WIN slice lived here -- tier-proximity serving, retired 2026-08-27.)
